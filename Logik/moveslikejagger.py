...
def bonde_træk_s(position):
    if position[0] == 1: #Da en bonde må flytte sig 2 på dens første træk spørger vi først efter om den står på den første flade
        if b.Bræt[position[0]+2][position[1]] == ' ' and Bræt[position[0]+1][position[1]] == '  ': #Derefter tjekker vi om de to pladser foran den er ledige ved at tjekke for x markeringer
        ...
        for placering in pladserforanh:
            if på_bord(placering):
                if pladserforanh.index(placering) % 2 == 0:
                    try:
                        if b.Bræt[position[0]][position[1]].hold == "s":
                            b.Bræt[position[0]][position[1]].kandræbes = True
                    except:
                        pass
                else:
                    if b.Bræt[position[0]][position[1]] == '':
                        b.Bræt[position[0]][position[1]] == 'x'
        ...
        for placering in retning: #Tjekker for alle individuelle placeringer i de her retninger
            if på_bord(placering): #Tjekker om de er på bordet for en sikkerhedsskyld
                if b.Bræt[placering[0]][placering[1]] == ' ': #Hvis der ikke står noget
                    ...
                    if b.Bræt[placering[0]][placering[1]].brikker.Brik.hold != b.Bræt[position[0]][position[1]].brikker.Brik.hold: #Hvis tårnets hold ikke er det samme som den brik der står på feltet
                        b.Bræt[placering[0]][placering[1]].brikker.Brik.kandræbes = True #Ændre den briks attribute kandræbes til at være sand
...
def løber_træk(position):
    ...
    for retning in kryds:
        for placering in retning:
            if på_bord(placering):
                if b.Bræt[placering[0]][placering[1]] == ' ':
                    b.Bræt[placering[0]][placering[1]] = 'x '
                else:
                    if b.Bræt[placering[0]][placering[1]].brikker.Brik.hold != b.Bræt[position[0]][position[1]].brikker.Brik.hold:
                        b.Bræt[placering[0]][placering[1]].brikker.Brik.kandræbes = True
