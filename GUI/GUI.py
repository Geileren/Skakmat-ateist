...
class Felt():
    ...
    def check_farve(self):
        if (self.x + self.y) % 2 == 0:  # tjekker om feltet skal være mørkt
            if self.brikker.index(self.billede) % 2 != 0:  # hvis feltet ikke er mørkt
                self.skift([self.brikker[self.brikker.index(self.billede) - 1],
                            self.type])  # skiftes til mørk version af samme brik
            ...
            if self.brikker.index(self.billede) % 2 != 1:  # hvis feltet ikke er lyst
                self.skift([self.brikker[self.brikker.index(self.billede) + 1],
                            self.type])  # skiftes til lys version af samme brik
            else:  # feltet har korrekt farve
                pass
    ...
    def tryk(self):
        ...
        elif self.type != "tom" and self.bræt.flyt_brik == False and mv.tjek_hold(self.bræt.runde, [self.y,
                                                                                                    self.x]) == None:  # hvis forkert farve brik vælges
            pass
...
class Bræt:
    ...
    def gen_start_lister(self):
        self.sort_start = [self.sort_tårn_lys, self.sort_springer_mørk, self.sort_løber_lys, self.sort_dronning_mørk,
                           self.sort_konge_lys, self.sort_løber_mørk, self.sort_springer_lys, self.sort_tårn_mørk]
        # liste svarrende til startpositionen for sorte brækker på række 8
        self.hvid_start = [self.hvid_tårn_mørk, self.hvid_springer_lys, self.hvid_løber_mørk, self.hvid_dronning_lys,
                           self.hvid_konge_mørk, self.hvid_løber_lys, self.hvid_springer_mørk, self.hvid_tårn_lys]
        ...
        for y in range(1, 9):
            for x in range(1, 9):
                if y == 1:  # tegner alle startbrækkerne i række 1
                    self.table_fields.append(Felt(self.root, self, self.hvid_start[x - 1], x - 1, y - 1, self.brækker,
                                                  self.type_liste[x - 1]))

                elif y == 2:
                    self.table_fields.append(
                        Felt(self.root, self, self.hvid_start_bønder[x % 2], x - 1, y - 1, self.brækker, "bonde"))

                elif y == 7:
                    self.table_fields.append(
                        Felt(self.root, self, self.sort_start_bønder[x % 2], x - 1, y - 1, self.brækker, "bonde"))

                elif y == 8:  # tegner alle startbrækkerne i række 8
                    self.table_fields.append(Felt(self.root, self, self.sort_start[x - 1], x - 1, y - 1, self.brækker,
                                                  self.type_liste[x - 1]))
...
def main():
    ...
    b = Bræt(root, gen_picts(path, 3))  # initialisere brættet
    b.lav()  # tegner brættet
    mainloop()
